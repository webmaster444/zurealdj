class WhoWeArePage < ActiveRecord::Base

  def content_attachments
    Attachment.where entity_id: self.id, entity_type: 'who_we_are_page_content'
  end
  ...
  def update_content_attachments
    ...
    Attachment.where(entity_id: nil, entity_type: "who_we_are_page_content").each do |attachment|
