class HelpCenterPage < ApplicationRecord

  def content_attachments
    Attachment.where entity_id: self.id, entity_type: 'help_center_page_content'
  end
  ...
  def update_content_attachments
    ...
    Attachment.where(entity_id: nil, entity_type: "help_center_page_content").each do |attachment|
