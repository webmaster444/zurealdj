class TermsNConditionsPage < ActiveRecord::Base

  def content_attachments
    Attachment.where entity_id: self.id, entity_type: 'terms_n_conditions_page_content'
  end
  ...
  def update_content_attachments
    ...
    Attachment.where(entity_id: nil, entity_type: "terms_n_conditions_page_content").each do |attachment|
